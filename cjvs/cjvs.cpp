...
uint8_t ReadByte(HANDLE hCom, LPDWORD lpRead) {
...
int SetupJVS(JVSKEY *jkey, HANDLE hCom, HANDLE hFile)
{
	...
	if (!fSuccess)
	{
		LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"SetupJVS - GetCommState failed with error %d.\r\n", GetLastError()));
		return (0);
	}
	...
	LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"BaudRate = %d, ByteSize = %d, Parity = %d, StopBits = %d\r\n", dcb.BaudRate, dcb.ByteSize, dcb.Parity, dcb.StopBits));
...
void ReadJVS(HANDLE hCom, HANDLE hFile, uint8_t *buffer, int size)
{
	...
	while (true) 
	{
		buf = ReadByte(hCom, &read);

		if (read == NULL) 
		{
			LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"ReadJVS - Read Error!\r\n"));
			break;
		}
		...
		if (buf == JVS_SYNC_CODE) 
		{
			...
			dest = ReadByte(hCom, &read);
			data_size = ReadByte(hCom, &read) - 1;

			for (i = 0; i < data_size; i++)
			{
				buffer[i] = ReadByte(hCom, &read);

				if (buffer[i] == JVS_ESCAPE_CODE)
				{
					buffer[i] = ReadByte(hCom, &read) + 1;
				}
...
int init_master(HANDLE hCom, HANDLE hFile)
{
	...
	if (jdata[0] != JVS_OK)
	{
		LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"Initialize Master - jdata[0] JVS NOT OK!\r\n"));
		return(0);
	}
	if (jdata[1] != JVS_OK)
	{
		LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"Initialize Master - jdata[1] JVS NOT OK!\r\n"));
		return(0);
	}
...
bool GetCoin(JVSKEY *jkey, INPUT *ip, HANDLE hCom, HANDLE hFile, int node)
{
	...
	//{
		//LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"GetCoin - jdata[0] JVS NOT OK!\r\n"));
		//return(0);
	//}

	//if (jdata[1] != JVS_OK)
	//{
		//LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"GetCoin - jdata[1] JVS NOT OK!\r\n"));
		//return(0);
	//}
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	hFile = CreateFile(_T("D:\\TypeXmulti\\ttxjvs.log"), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	hCom = CreateFile(pcCommPort, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);

	if (hCom == INVALID_HANDLE_VALUE)
	{
		LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"Main - Could not open COM port! Error: %d.\r\n", GetLastError()));
		return (1);
	}

	SetupJVS(&jkey, hCom, hFile);
	SetupJVSKEY(&jkey, &ip, &di);
	LogEvent(hFile, ltext, swprintf_s(ltext, sizeof(ltext), L"Main - COM Setup.\r\n"));
